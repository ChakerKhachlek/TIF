<div>
    <div class="container-fluid justify-content-center text-center text-white">
        ...
        @include('livewire.dashboard.Styles.deleteStyle')
        ...
        @if($updateMode)
            <div class="d-flex justify-content-center">
                ...
                @include('livewire.dashboard.Styles.updateStyle')

            </div>
        @elseif($createMode)
            <div class="d-flex justify-content-center">
                @include('livewire.dashboard.Styles.createStyle')
            </div>
        @endif
        @if (count($errors) > 0)
    ...
    @endif
        ...
        <div class="row d-flex justify-content-center pt-4">
            <div class="col-md-4">
                <div class="h3 text-white">Styles List ({{$data->count()}})</div>
            </div>
                ...
                <table class="table table-bordered table-condensed ">
                    ...
                    @foreach($data as $row)
                        <tr>
                            <td>{{$row->id}}</td>
                            <td>{{$row->name}}</td>
                            <td><img src="{{url('storage/styles_images/'.$row->style_img_link) }}" height="80px" width="80px"></img></td>
                            ...
                            <td width="100">
                                ...
                                <button type="button" rel="tooltip" class="btn btn-danger btn-sm btn-icon"  wire:click="deleteStyleMode('{{ $row->id }}' )"
                                    data-toggle="modal" data-target="#deleteStyleModal">
    ...
    Livewire.on('style-created', event => {
        toastr.success(event);
    });
    Livewire.on('style-updated', event => {
        toastr.success(event);
    });

    Livewire.on('style-deleted', event => {
        $("#deleteStyleModal").modal('hide');
